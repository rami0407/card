...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBackToRoles, lang, currentTeacher }) => {
  ...
  const handleSaveMysteryText = async (cardId: string, text: string) => {
    try {
      await updateCardMysteryText(cardId, text);
      setCards(prev => prev.map(c => c.id === cardId ? { ...c, mysteryText: text } : c));
    } catch (err) {
      console.error("Error saving mystery text:", err);
    }
  };
              ...
              <div className="cards-display-column">
                ...
                {cards.length === 0 ? (
                  ...
                  <div className="cards-thumbnail-grid">
                    {cards.map((card) => (
                      <div key={card.id} className="card-wrapper" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <div className="card-thumbnail-item" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative', width: '100%' }}>
                        ...
                        {card.isMystery && (
                          <input
                            type="text"
                            className="mystery-text-input"
                            defaultValue={card.mysteryText || ''}
                            placeholder={t('mystery_text_placeholder', lang)}
                            onBlur={(e) => handleSaveMysteryText(card.id, e.target.value)}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
